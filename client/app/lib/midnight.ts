...
export function createZkConfigProvider(zkPath: string): FetchZkConfigProvider<any> {
  return new FetchZkConfigProvider<any>(
...
async function bech32ToHex(encoded: string): Promise<string> {
  const { MidnightBech32m } = await import("@midnight-ntwrk/wallet-sdk-address-format");
  const parsed = MidnightBech32m.parse(encoded);
  const raw = parsed.data instanceof Uint8Array ? parsed.data : new Uint8Array(parsed.data);
  return toHex(raw);
}

export async function createConnectedSession(api: any, zkPath: string): Promise<ConnectedSession> {
  const [config, unshieldedAddress, shieldedAddress] = await Promise.all([
    api.getConfiguration(),
    api.getUnshieldedAddress(),
    api.getShieldedAddresses(),
  ]);
  const pkHex = await bech32ToHex(shieldedAddress.shieldedCoinPublicKey);
